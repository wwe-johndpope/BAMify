...
extension BAMify {
	open class MetaDeletionQuery: GraphQL.AbstractQuery, GraphQLQuery {
		public typealias Response = MetaDeletion

		@discardableResult
		open func errorMessages(aliasSuffix: String? = nil) -> MetaDeletionQuery {
			addField(field: "errorMessages", aliasSuffix: aliasSuffix)
			return self
		}

		@discardableResult
		open func deletionsAttempted(aliasSuffix: String? = nil) -> MetaDeletionQuery {
			addField(field: "deletionsAttempted", aliasSuffix: aliasSuffix)
			return self
		}

		@discardableResult
		open func deletionsSucceeded(aliasSuffix: String? = nil) -> MetaDeletionQuery {
			addField(field: "deletionsSucceeded", aliasSuffix: aliasSuffix)
			return self
		}
	}

	open class MetaDeletion: GraphQL.AbstractResponse, GraphQLObject {
		public typealias Query = MetaDeletionQuery
		...
		open override func deserializeValue(fieldName: String, value: Any) throws -> Any? {
			...
			switch fieldName {
				...
				guard let value = value as? [Any] else {
					throw SchemaViolationError(type: MetaDeletion.self, field: fieldName, value: fieldValue)
				}
		...
		open var typeName: String { return "MetaDeletion" }
